    ...
    def parse_data(self, data, **kwargs) -> str:
        ...
        if server_data:
            ...
            self.version.user.game_access_token = (
                server_data.get("GameAccessToken", "") or ""
            )
            self.version.user.authentication_token = (
                server_data.get("AuthenticationToken", "") or ""
            )
            self.version.user.remember_me_token = (
                server_data.get("RememberMeToken", "") or ""
            )
            self.version.user.support_url = server_data.get("SupportUrl", "") or ""
            self.version.user.save(
                update_fields=[
                    "player_id",
                    "game_access_token",
...
class SQLDefinitionHelper(ImportHelperMixin):
    ...
    def _read_sqlite(self, model, remote_table_name, mapping, cur):
        ...
        remote_fields_in_order = ",".join([mapping[f] for f in local_fields])
        sql = f"SELECT {remote_fields_in_order} FROM {remote_table_name}"

        for row in cur.execute(sql):
            param = {local_fields[i]: row[i] for i in range(len(local_fields))}
    ...
    def _read_article(self, cur):
        ...
        self._read_sqlite(
            model=model, remote_table_name=remote_table_name, mapping=mapping, cur=cur
        )
