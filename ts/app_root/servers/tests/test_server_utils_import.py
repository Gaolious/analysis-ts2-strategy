...
def test_server_import_endpoint(
    multidb, filename, fixture_crawling_get, fixture_crawling_post
):
    class FakeResp(AbstractFakeResp):
        text = (settings.DJANGO_PATH / "fixtures" / "endpoints" / filename).read_text(
            "utf-8"
        )
    ...
    names = [
        ...
        "payment",
        "command_processing",
        "command_processing_collection",
        "support_url",
        "definitions",
        "start_game",
        "login_restore_url",
        "restore_decision_url",
        "store_notification_token",
        "update_device_id",
        "impersonate_link",
        "payments",
        "leaderboard",
        ...
        "applogs",
        "firebase_db",
        "payment_obfuscation_public_key",
        "ts2feedback",
        "facebook",
        "twitter",
        "forum",
...
def test_server_import_sql_definition(
    ...
    filename,
    sqlite_filename,
    fixture_crawling_get,
    fixture_crawling_post,
    fixture_download_file,
):
    class FakeResp(AbstractFakeResp):
        text = (settings.DJANGO_PATH / "fixtures" / "definition" / filename).read_text(
            "utf-8"
        )

    sqllite_filepath = (
        settings.DJANGO_PATH / "fixtures" / "definition" / sqlite_filename
    )
    ...
    user = User.objects.create_user(
        username="test", android_id="test", game_access_token="a", player_id="1"
    )
    version = RunVersion.objects.create(user_id=user.id, level_id=1)
    EndPoint.objects.create(
        name=EndPoint.ENDPOINT_DEFINITION,
        name_hash=hash10(EndPoint.ENDPOINT_DEFINITION),
        url="a",
    )
...
def test_utils_login_helper(
    multidb, filename, remember_me_token, fixture_crawling_get, fixture_crawling_post
):
    class FakeResp(AbstractFakeResp):
        text = (settings.DJANGO_PATH / "fixtures" / "login" / filename).read_text(
            "utf-8"
        )
    ...
    # prepare
    user = User.objects.create_user(
        username="test", android_id="test", remember_me_token=remember_me_token
    )
    version = RunVersion.objects.create(user_id=user.id, level_id=1)
    EndPoint.objects.create(
        name=EndPoint.ENDPOINT_LOGIN, name_hash=hash10(EndPoint.ENDPOINT_LOGIN), url="a"
    )
