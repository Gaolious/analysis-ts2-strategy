...
from app_root.strategies.commands import (
    ...
    ContractAcceptCommand,
    send_commands,
    TrainSendToDestinationCommand,
    FactoryCollectProductCommand,
    FactoryOrderProductCommand,
    ContractAcceptWithVideoReward,
    GameSleep,
    GameWakeup,
)
...
from app_root.strategies.managers import (
    ...
    factory_find_destination_and_factory_only_products,
    factory_find_product_orders,
    warehouse_countable,
    article_find_contract,
    article_find_product,
    article_find_destination,
    warehouse_max_capacity,
    trains_loads_amount_article_id,
...
def build_article_sources(version: RunVersion) -> Dict[int, ArticleSource]:
    ...
    countables = warehouse_countable(version=version, basic=True, **param)
    article_source = {}

    for article_id, (article, amount) in countables.items():
        if article_id not in article_source:
            article_source.update({article_id: ArticleSource(article=article)})

        contract_list = article_find_contract(
            version=version, article_id=article_id, available_only=False
        ).get(article_id, [])
        for contract in contract_list:
            article_source[article_id].add_contract(contract=contract)

        destination_list = article_find_destination(
            version=version, article_id=article_id
        ).get(article_id, [])
        for destination in destination_list:
            article_source[article_id].add_destination(dest=destination)

        products_list = article_find_product(
            version=version, article_id=article_id
        ).get(article_id, [])
...
def build_factory_strategy(version: RunVersion) -> Dict[int, FactoryStrategy]:
    ...
    for player_factory in factory_find_player_factory(version=version):
        ...
        completed, processing, waiting = factory_find_product_orders(
            version=version, factory_id=int(player_factory.factory_id)
        )
        ...
        if len(factory_product_list) == 1:
            ...
            strategy.strategy_article_count.update(
                {article_id: player_factory.slot_count * 2}
            )

        elif len(factory_product_list) > 1:
            products = sorted(
                factory_product_list,
                key=lambda x: (x.craft_time, x.article_amount),
                reverse=True,
            )
...
def get_factory_materials(
    version: RunVersion, factory_strategy_dict: Dict[int, FactoryStrategy]
) -> Material:
    """
        Factory의 개수를 채운다.
    ...
    for factory_id, strategy in factory_strategy_dict.items():
        ...
        for product in strategy.factory_only_products:
            ...
            need_more_count = required_count - (
                processing_count + waiting_count + completed_count
            )
            ...
            for (
                required_article_id,
                required_article_amount,
            ) in product.conditions_to_article_dict.items():
                material.add(
                    article_id=required_article_id, amount=required_article_amount
                )

    return material


def command_collect_factory_product_redundancy(
    version: RunVersion,
    factory_strategy_dict: Dict[int, FactoryStrategy],
    ...
    for factory_id, strategy in factory_strategy_dict.items():
        ...
        for product in strategy.factory_only_products:
            ...
            warehouse_amount = warehouse_get_amount(
                version=version, article_id=product.article_id
            )
            product_amount = product.article_amount

            if warehouse_amount + product_amount < avg_amount:
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Warehouse[{warehouse_amount} | In Factory[{cnt}*{product.article_amount}] Try Collect"
                )
                material = Material()
...
def command_order_product_in_factory(
    version: RunVersion, product: TSProduct, count: int
):
    article_id = int(product.article_id)
...
def command_send_destination(
    version: RunVersion, destination: TSDestination, amount: int
):
    ...
    possible_train = list(
        trains_find(version=version, **destination.requirements_to_dict, is_idle=True)
    )
    ...
    train_loads_amount = trains_loads_amount_article_id(
        version=version, article_id=required_article_id
    )
    ...
    for train in possible_train:
        ...
        print(
            f"    - Send Destination Dest Location ID #{destination.location_id} / Train[{train.capacity()}] / Worker = Normal[{normal_workers}], Union[{union_workers}]"
        )
...
def command_factory_strategy(
    ...
    for factory_id, strategy in factory_strategy_dict.items():
        ...
        strategy.update(completed=completed, processing=processing, waiting=waiting)
        player_factory = factory_find_player_factory(
            version=version, factory_id=factory_id
        )
        if isinstance(player_factory, list):
            player_factory = player_factory[0]
        ...
        for product in strategy.factory_only_products:
            ...
            if need_more_count < 0:
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Need {need_more_count} | Try Collect"
                )
                command_collect_factory(
                    version=version, product=product, count=-need_more_count
                )
                continue

            if need_more_count == 0:
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Satisfied | PASS"
                )
                continue

            available_slot = player_factory.slot_count - (
                len(processing) + len(waiting)
            )
            if available_slot <= 0:
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Need {need_more_count} | Available Slot:{available_slot} | Reach to Max Slot"
                )
                continue
            # if len(processing) > 0:
            ...
            if check_all_has_in_warehouse(version=version, requires=material):
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Need {need_more_count} | Try Order"
                )
                command_order_product_in_factory(
                    version=version, product=product, count=1
                )
            else:
                print(
                    f"    - Factory: {product.factory} | Article[#{product.article_id}|{product.article.name}] | Need {need_more_count} | Not Enough Material"
                )
...
def get_destination_materials(version: RunVersion) -> Material:
    ...
    warehouse_countable_articles = warehouse_countable(
        version=version, basic=True, event=False, union=False
    )

    avg_amount = warehouse_avg_count(
        version=version, warehouse_countable_articles=warehouse_countable_articles
    )
    ...
    for article_id, (article, has_amount) in warehouse_countable_articles.items():
        ...
        destinations = article_find_destination(
            version=version, article_id=article_id
        ).get(article_id, [])
        if not destinations:
            continue
...
def check_all_has_in_warehouse(version: RunVersion, requires: Material) -> bool:
    for required_article_id, required_amount in requires.items():
        if required_amount > warehouse_get_amount(
            version=version, article_id=required_article_id
        ):
            return False
    return True
...
def command_trade_contract(version: RunVersion, contract: PlayerContract):
    ...
    cmd_list = [
        ...
        ContractAcceptWithVideoReward(
            version=version, contract=contract, accept_at=accept_at
        ),
    ]
...
def command_collect_contract_if_possible(
    version: RunVersion,
    required_article_id: int,
    required_amount: int,
    article_source: Dict[int, ArticleSource],
):
    # 계약서 사용 가능하면
    ...
    for contract in source.contracts:
        ...
        if required_amount <= warehouse_amount:
            print(
                f"    - #{required_article_id} : required:{required_amount} <= {warehouse_amount} | PASS"
            )
            break
        if not contract.is_available(version.now):
            print(
                f"    - #{required_article_id} : required:{required_amount} <= {warehouse_amount} | Not Available. PASS"
            )
            continue
...
def command_collect_destination_if_possible(
    version: RunVersion,
    required_article_id: int,
    required_amount: int,
    article_source: Dict[int, ArticleSource],
    ...
    for destination in source.destinations:
        possible_train = list(
            trains_find(
                version=version, **destination.requirements_to_dict, is_idle=True
            )
        )
        possible_train.sort(key=lambda x: x.capacity(), reverse=True)
...
def command_collect_factory(version: RunVersion, product: TSProduct, count: int):
    ...
    completed, processing, waiting = factory_find_product_orders(
        version=version, factory_id=factory_id, article_id=article_id
    )
...
def command_collect_factory_if_possible(
    version: RunVersion,
    required_article_id: int,
    required_amount: int,
    article_source: Dict[int, ArticleSource],
    ...
    for product in source.products:
        ...
        for order in completed:
            ...
            if required_amount <= warehouse_amount:
                print(
                    f"    - Factory: {product.factory} | required amount={required_amount} <= warehouse={warehouse_amount}| PASS"
                )
                break
            ...
            if used + order.amount > max_capacity:
                print(
                    f"    - Factory: {product.factory} | Try Collect Item Index={order.index} | amount={required_amount} + used Warehouse[{used}] > max Warehouse[{max_capacity}] | PASS"
                )
                continue

            print(
                f"    - Factory: {product.factory} | Try Collect Item Index={order.index} | required amount={required_amount} > warehouse={warehouse_amount}"
            )
            order.refresh_from_db()
...
def command_collect_materials_if_possible(
    version: RunVersion, requires: Material, article_source: Dict[int, ArticleSource]
):
    """
    필요한 재료만큼 수집
    ...
    """
    for required_article_id, required_amount in requires.items():
        command_collect_contract_if_possible(
            version=version,
            required_article_id=required_article_id,
            required_amount=required_amount,
            article_source=article_source,
        )

        command_collect_destination_if_possible(
            version=version,
            required_article_id=required_article_id,
            required_amount=required_amount,
            article_source=article_source,
...
def material_strategy_add_queue(
    ...
    for required_article_id, required_article_amount in requires.items():
        ...
        if source.contracts:
            ...
            if required_article_amount * multiply < warehouse_get_amount(
                version=version, article_id=required_article_id
            ):
                ret.append(
                    f"""{'  ' * depth} - Required:[{source.article}] - Enough material (more than x2)| PASS"""
                )
                continue

            s = 0
            for contract in source.contracts:
                if s >= required_article_amount:
                    break
                amount = sum(
                    [
                        v
                        for k, v in contract.reward_to_article_dict.items()
                        if k == required_article_id
                    ]
                )
                s += amount
                strategy.push_contract(contract)
                ret.append(
                    f"""{'  ' * depth} - Required:[{source.article}] - Contract Slot[{contract.slot}][{amount} 개]"""
                )
            ...
            remain_warehouse_amount = warehouse_amount - strategy.get_used_warehouse(
                required_article_id
            )
            used_warehouse_amount = min(
                required_article_amount, remain_warehouse_amount
            )
            strategy.add_used_warehouse(
                article_id=required_article_id, amount=used_warehouse_amount
            )
            required_article_amount -= used_warehouse_amount

            if required_article_amount > 0:
                ret.append(
                    f"""{'  ' * depth} - Required:[{source.article}] - Destination[{source.destinations[0].id}][{required_article_amount} 개]"""
                )
                strategy.push_destination(
                    source.destinations[0], required_article_amount
                )

        elif source.products:
            ret.append(
                f"""{'  ' * depth} - Required:[{source.article}] - Product[{required_article_amount} 개]"""
            )
...
def material_strategy_contract(
    ...
    while not strategy.empty_contract():
        ...
        for required_article_id, required_article_amount in material.items():
            ...
            if article:
                required_material_name.append(f"[{article}:{required_article_amount}개]")
            if (
                warehouse_amount
                < required_article_amount
                + strategy.get_used_warehouse(required_article_id)
            ):
                all_satisfied = False
                break
        ...
        if all_satisfied:
            print(
                f"""{'  ' * depth} - Check Contract Slot[{contract.slot}] (Require:{'|'.join(required_material_name)}) - You Can collect"""
            )
            strategy.add_collectable_contract(contract)
        else:
            print(
                f"""{'  ' * depth} - Check Contract Slot[{contract.slot}] (Require:{'|'.join(required_material_name)}) - Not Enough"""
            )
...
def material_strategy_factory(
    ...
    for player_factory in player_factories:
        ...
        product_orders.update(
            {
                factory_id: factory_find_product_orders(
                    version=version, factory_id=factory_id
                )
            }
        )
    ...
    while not strategy.all_empty_factory():
        ...
        for player_factory in player_factories:
            ...
            while not strategy.empty_factory_queue(factory_id=factory_id):
                product, required_article_amount = strategy.pop_factory(
                    factory_id=factory_id
                )
                required_article_id = product.article_id

                completed_amount = sum(
                    [c.amount for c in completed if c.article_id == required_article_id]
                )
                processing_amount = sum(
                    [
                        c.amount
                        for c in processing
                        if c.article_id == required_article_id
                    ]
                )
                waiting_amount = sum(
                    [c.amount for c in waiting if c.article_id == required_article_id]
                )
                uncompleted_amount = processing_amount + waiting_amount

                completed_amount -= strategy.get_used_collectable_factory(
                    product=product
                )
                uncompleted_amount -= strategy.get_used_uncollectable_factory(
                    product=product
                )
                ...
                remain_warehouse_amount = (
                    warehouse_amount - strategy.get_used_warehouse(required_article_id)
                )
                ...
                if used_warehouse_amount > 0:
                    strategy.add_used_warehouse(
                        product.article_id, required_article_amount
                    )
                    remain_warehouse_amount -= used_warehouse_amount
                    required_article_amount -= used_warehouse_amount

                used_completed_product_amount = min(
                    completed_amount, required_article_amount
                )
                if used_completed_product_amount > 0:
                    strategy.add_collectable_factory(
                        product, used_completed_product_amount
                    )
                    completed_amount -= used_completed_product_amount
                    required_article_amount -= used_completed_product_amount

                used_uncompleted_product_amount = min(
                    uncompleted_amount, required_article_amount
                )
                if used_uncompleted_product_amount > 0:
                    strategy.add_uncollectable_factory(
                        product, used_uncompleted_product_amount
                    )
...
def expand_material_strategy(
    ...
    :return:
    """
    warehouse_count = warehouse_countable(
        version=version, basic=True, event=False, union=True
    )
...
def command_material_strategy(
    ...
    for destination, amount in strategy.destination_queue:
        ...
        if amount > 0:
            command_send_destination(
                version=version, destination=destination, amount=amount
            )
    ...
    for factory_id in strategy.factory_uncollectable:
        for product, amount in strategy.factory_uncollectable[factory_id]:
            cnt = math.ceil(amount / product.article_amount)
            command_order_product_in_factory(
                version=version, product=product, count=cnt
            )
...
def command_ship_trade(
    ...
    command_material_strategy(version=version, strategy=strategy)
