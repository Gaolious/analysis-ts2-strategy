...
from app_root.players.mixins import (
    PlayerBuildingMixin,
    PlayerDestinationMixin,
    PlayerFactoryMixin,
    PlayerFactoryProductOrderMixin,
    PlayerJobMixin,
    PlayerContractListMixin,
    PlayerContractMixin,
    PlayerGiftMixin,
    PlayerLeaderBoardMixin,
    PlayerLeaderBoardProgressMixin,
    PlayerTrainMixin,
    PlayerWarehouseMixin,
    PlayerWhistleItemMixin,
    PlayerWhistleMixin,
    PlayerAchievementMixin,
    PlayerDailyRewardMixin,
    PlayerMapMixin,
    PlayerQuestMixin,
    PlayerVisitedRegionMixin,
    PlayerShipOfferMixin,
    PlayerCompetitionMixin,
    PlayerUnlockedContentMixin,
    PlayerOfferContainerMixin,
    PlayerDailyOfferMixin,
    PlayerDailyOfferItemMixin,
    PlayerCityLoopParcelMixin,
    PlayerCityLoopTaskMixin,
)
...
class PlayerFactoryProductOrder(
    PlayerFactoryProductOrderMixin, BaseModelMixin, TimeStampedMixin
):
    class Meta:
...
class PlayerJob(PlayerJobMixin, BaseModelMixin, TimeStampedMixin):
    ...
    def __str__(self):
        ...
        if self.is_union_job:
            ret.append(
                f"Progress: {self.current_progress}/{self.required_amount} ({self.current_progress / self.required_amount * 100:.2f} %)"
            )
        else:
            ret.append(
                f"Progress: {self.current_article_amount}/{self.required_amount} ({self.current_article_amount/self.required_amount*100:.2f} %)"
            )

        ret.append(
            f"Required: #{self.required_article.id}|{self.required_article.name}"
        )
        return " / ".join(ret)
...
class PlayerLeaderBoardProgress(
    PlayerLeaderBoardProgressMixin, BaseModelMixin, TimeStampedMixin
):
    class Meta:
...
class PlayerUnlockedContent(
    PlayerUnlockedContentMixin, BaseModelMixin, TimeStampedMixin
):
    class Meta:
...
class PlayerDailyOfferContainer(
    PlayerOfferContainerMixin, BaseModelMixin, TimeStampedMixin
):
    class Meta:
