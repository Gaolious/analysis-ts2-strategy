...
def test_utils_initdata_helper(
    multidb, filename, fixture_crawling_get, fixture_crawling_post
):
    class FakeResp(AbstractFakeResp):
        text = (settings.DJANGO_PATH / "fixtures" / "init_data" / filename).read_text(
            "utf-8"
        )
    ...
    EndPoint.objects.create(
        name=EndPoint.ENDPOINT_INIT_DATA_URLS,
        name_hash=hash10(EndPoint.ENDPOINT_INIT_DATA_URLS),
        url="a",
    )
...
def test_utils_initdata_competition(
    multidb, filename1, filename2, fixture_crawling_get, fixture_crawling_post
):
    class FakeResp1(AbstractFakeResp):
        text = (
            settings.DJANGO_PATH / "fixtures" / "competitions" / filename1
        ).read_text("utf-8")

    class FakeResp2(AbstractFakeResp):
        text = (
            settings.DJANGO_PATH / "fixtures" / "competitions" / filename2
        ).read_text("utf-8")
    ...
    fixture_crawling_get.side_effect = [FakeResp1(), FakeResp2()]
