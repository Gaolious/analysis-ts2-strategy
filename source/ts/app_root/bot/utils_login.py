...
class Login(BaseBotRequest):
    ...
    def parse_data(self, data, user: User) -> str:
        ...
        if server_data:
            user.player_id = server_data.get('PlayerId', '') or ''
            user.game_access_token = server_data.get('GameAccessToken', '') or ''
            user.authentication_token = server_data.get('AuthenticationToken', '') or ''
            user.remember_me_token = server_data.get('RememberMeToken', '') or ''
            user.device_id = server_data.get('DeviceId', '') or ''
            user.support_url = server_data.get('SupportUrl', '') or ''
